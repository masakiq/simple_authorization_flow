...
class Authorization < WEBrick::HTTPServlet::AbstractServlet
  def do_GET request, response
    ...
    if response_type == 'code' && client_id == ENV['CLIENT_ID'] && redirect_uri == "#{ENV['SOCIAL_URI']}/callback"
      response.status = 200
...
class Token < WEBrick::HTTPServlet::AbstractServlet
  def do_GET request, response
    ...
    if grant_type == 'authorization_code' && request.query['code'] == ENV['AUTH_CODE'] && redirect_uri == "#{ENV['SOCIAL_URI']}/callback"
      response.status = 200
      response['Content-Type'] = 'text/plain'
